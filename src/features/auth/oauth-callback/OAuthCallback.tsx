...
export function OAuthCallback() {
  ...
  const content = getErrorContent(errorCode ?? providerError, t)
  ...
  return (
    <section className={styles.root} aria-busy={isLoading || undefined}>
      {!errorCode && !providerError ? (
        <>
          <Title level="h1">{t.auth.oauth.completingTitle}</Title>
          <Text color="var(--color-text-secondary)">{t.auth.oauth.completingMessage}</Text>
        </>
      ) : (
        <>
          ...
          <Button asChild className={styles.action}>
            <Link href={authRoutes.signIn}>{t.auth.oauth.backToSignIn}</Link>
          </Button>
        </>
      )}
    </section>
